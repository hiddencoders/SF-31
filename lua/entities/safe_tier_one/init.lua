...
function ENT:Initialize()	
	...
	self:SetAmount(0)
		...
		self.Safe.NeedToBeOpened["__MAX"] = 1 --[[number of values in NeedToBeOpened, because we are lazy.]]
		...
		self.Safe.ToOpenedWith["crowbar"] = {3,7}			// How many times you have to try open the safe {minimum, maximum} with this. If it's between you MIGHT get it open. Keep the curly brackets!
		...
		self.InterestRate = 3							// Interest rate in percentage
	...
	--timer.Simple(0.1, function() if self:IsValid() then self:SetModelScale( self:GetModelScale() * 0.33, 0 ) end end)
end
...
function ENT:SpawnFunction( ply, tr )
    ...
    ent:Spawn()
	ent:SetModelScale( ent:GetModelScale() * 0.33, 0 )
    ent:Activate()
